...
namespace
{
    ...
    intrusive_ptr<QueryPlan> LexRange(const string &lo, const string &hi)
    {
        ...
        assert(hi.size() != 0);
        if (lo[0] < hi[0])
            out->insert(QueryPlan::value_type(pair<uchar, uchar>(lo[0], hi[0] - 1), (QueryPlan *)0));
        out->insert(QueryPlan::value_type(pair<uchar, uchar>(hi[0], hi[0]),
    ...
    intrusive_ptr<QueryPlan> CClass(CharClass *cc)
    {
        ...
        {
            if (i->hi < Runeself)
                k->insert(QueryPlan::value_type(pair<uchar, uchar>(i->lo, i->hi),
                                                (QueryPlan *)0));
            else
            {
                alternate_cache cache;
                k = Alternate(cache, k, LexRange(RuneToString(i->lo), RuneToString(i->hi)));
            }
        }
              ...
              intrusive_ptr<QueryPlan> rnext)
    {
        if (intersects(left, right))
        {
            ...
            if (left.first < right.first)
            {
                out->insert(make_pair(make_pair(left.first,
                                                right.first - 1),
                                      lnext));
            ...
            {
                out->insert(make_pair(make_pair(right.first,
                                                left.first - 1),
                                      rnext));
            ...
            out->insert(make_pair(make_pair(left.first, end),
