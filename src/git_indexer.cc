...
git_indexer::git_indexer(code_searcher *cs,
    ...
    : cs_(cs), repo_(0), repopath_(repopath), name_(name), metadata_(metadata), walk_submodules_(walk_submodules)
{
    int err;
...
void git_indexer::walk(const string &ref)
{
    ...
    string version = FLAGS_revparse ? strdup(git_oid_tostr(oidstr, sizeof(oidstr), git_commit_id(commit))) : ref;
